...
동일한 소스 코드인데 왜 이렇게 차이가 나는 것일까?

여기서 바로 CDN의 차이가 나타나는 것 같다. s3는 endpoint에서 반환하는것이니 우리의 요청이 그곳 해당 Region까지 도달해야한다. 그러다 보니 시간이 오래 걸릴 수 밖엥 없다. 더 찾아 보니 이것 외에도 프로토콜 압축 지원 부분에서도 차이가 많이 나는 거 같다. 이 정도 성능차이라면 cloud front를 사용하지 않을 이유가 없다.

SI(Speed Index) 와 LCP(Largest Contentful Paint) 사용자 체감 지표 라는 것, 

### SI 
얼마나 빨리 화면이 채워지는 지 특히, si가 s3가 정말 처참할 정도로 성능이 안나왔는데 위에 표처럼 저런것들이 하나씩 쌓이다 보니. 정말 성능에서 엉망으로 나타나지 않았나 생각해본다.

### LCP
화면에서 가장 큰 요소가 얼마나 빨리 그려지는지 에 대한 지표

재미있는 사실은 처음 로드 할때는 cloud front 가 s3보다 성능이 좋지 않았다. 첫로드가 s3보다 cloudfront가 오래걸린다. 당연히 해당 파일과 데이터를 캐싱하는 과정에서 시간이 좀  걸리는 거 같다!
